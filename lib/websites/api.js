...
export async function fetchUserCategoriesTree() {
  const response = await authApiRequest(`${BASE}/categories/tree`);
  const data = await response.json();
  if (!response.ok) throw new Error(data.error || 'Failed to fetch category tree');
  return data.categories || [];
}

export async function reorderCategories(categories) {
  const response = await authApiRequest(`${BASE}/categories/reorder`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
export async function fetchSiteCategories(siteId) {
  const response = await authApiRequest(`${BASE}/sites/${siteId}/categories`);
  const data = await response.json();
  if (!response.ok) throw new Error(data.error || 'Failed to fetch site categories');
  return data.categories || [];
}

export async function updateSiteCategoryVisibility(siteId, visibility) {
  const response = await authApiRequest(`${BASE}/sites/${siteId}/categories/visibility`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ visibility })
  });
  const data = await response.json();
  if (!response.ok) throw new Error(data.error || 'Failed to update category visibility');
  return data;
}
...
export async function startDomainValidation(siteId, customDomain) {
  const response = await authApiRequest(`${BASE}/domains/start-validation`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function retryDomainValidation(siteId) {
  const response = await authApiRequest(`${BASE}/domains/retry-validation/${siteId}`, {
    method: 'POST'
  });
  const data = await response.json();
...
export async function cancelDomainValidation(siteId) {
  const response = await authApiRequest(`${BASE}/domains/cancel-validation/${siteId}`, {
    method: 'POST'
  });
  const data = await response.json();
...
export async function removeCustomDomain(siteId) {
  const response = await authApiRequest(`${BASE}/domains/remove/${siteId}`, {
    method: 'DELETE'
  });
  const data = await response.json();
...
export async function selectWebsitesTier(body) {
  const response = await authApiRequest(`${BASE}/subscription/select-tier`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body)
  });
...
export async function acceptWebsitesTerms(termsVersionId) {
  const response = await authApiRequest(`${BASE}/subscription/terms-accept`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function changeWebsitesTier(body) {
  const response = await authApiRequest(`${BASE}/subscription/change-tier`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body)
  });
...
export async function confirmWebsitesTierChange(body) {
  const response = await authApiRequest(`${BASE}/subscription/confirm-tier-change`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body)
  });
  const data = await response.json();
  if (!response.ok) throw new Error(data.error || 'Failed to confirm tier change');
  return data;
}

export async function cancelWebsitesSubscription(body = {}) {
...
export async function confirmWebsitesCancellation(body) {
  const response = await authApiRequest(`${BASE}/subscription/confirm-cancellation`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body)
  });
...
export async function resolveSubdomain(subdomain) {
  const response = await fetch(getApiUrl(`${BASE}/resolve/${subdomain}`));
  const data = await response.json();
...
export async function resolveSubdomainProducts(subdomain, params = {}) {
  ...
  const url = getApiUrl(`${BASE}/resolve/${subdomain}/products${queryString ? `?${queryString}` : ''}`);
...
export async function resolveSubdomainArticles(subdomain, params = {}) {
  const queryString = new URLSearchParams(params).toString();
  const url = getApiUrl(`${BASE}/resolve/${subdomain}/articles${queryString ? `?${queryString}` : ''}`);
  const response = await fetch(url);
  const data = await response.json();
...
export async function resolveSubdomainCategories(subdomain) {
  const response = await fetch(getApiUrl(`${BASE}/resolve/${subdomain}/categories`));
  const data = await response.json();
  if (!response.ok) throw new Error(data.error || 'Failed to fetch categories');
  return data;
}
...
export async function checkSubdomainAvailability(subdomain) {
  const response = await fetch(getApiUrl(`${BASE}/check-subdomain/${subdomain}`));
  const data = await response.json();
  return data;
}

export async function resolveCustomDomain(domain) {
  const response = await fetch(getApiUrl(`${BASE}/resolve-custom-domain/${domain}`));
  const data = await response.json();
...
export async function fetchSiteAddonsPublic(siteId) {
  const response = await fetch(getApiUrl(`${BASE}/sites/${siteId}/addons`));
  const data = await response.json();

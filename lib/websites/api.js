...
export async function confirmWebsitesTierChange(body) {
  const response = await authenticatedApiRequest(`${BASE}/subscription/confirm-tier-change`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body)
  });
...
export async function confirmWebsitesCancellation(body) {
  const response = await authenticatedApiRequest(`${BASE}/subscription/confirm-cancellation`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body)
  });

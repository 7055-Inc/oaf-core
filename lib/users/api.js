 ...
 * Used for public profile pages and artist info on product pages
 * @param {number|string} userId - User ID
 * @returns {Promise<Object>} Public profile data
 */
export async function getPublicProfile(userId) {
  const response = await fetch(getApiUrl(`${USERS_BASE}/${userId}`));
  const data = await response.json();
  if (!data.success) throw new Error(data.error?.message || 'Profile not found');
  return data.data;
}
...
// =============================================================================
// PUBLIC ARTISTS (for carousels, featured sections)
// =============================================================================

/**
 * Get public artists for display (no auth required)
 * Used for homepage carousel, featured artists sections
 * @param {Object} options - Query options
 * @param {number} options.limit - Max results (default 50)
 * @param {boolean} options.random - Randomize order (default true)
 * @returns {Promise<Array>} Array of artist profiles
 */
export async function getPublicArtists({ limit = 50, random = true } = {}) {
  const params = new URLSearchParams({ limit, random });
  const response = await fetch(getApiUrl(`${USERS_BASE}/artists?${params}`));
  const data = await response.json();
  if (!data.success) throw new Error(data.error?.message || 'Failed to fetch artists');
  return data.data;
}

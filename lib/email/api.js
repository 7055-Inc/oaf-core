 ...
 */
export async function getTemplateDefault(id) {
  const response = await authenticatedApiRequest(
    getApiUrl(`${EMAIL_BASE}/templates/${id}/default`),
    { method: 'GET' }
  );

  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new Error(error.error || 'Failed to fetch template default');
  }
 ...
 */
export async function resetTemplateToDefault(id) {
  const response = await authenticatedApiRequest(
    getApiUrl(`${EMAIL_BASE}/templates/${id}/reset`),
    { method: 'POST' }
  );

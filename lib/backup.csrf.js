...
export async function fetchCsrfToken() {
  try {
    ...
    const response = await fetch('https://api.brakebee.com/csrf-token', {
      method: 'GET',
...
export async function refreshAuthToken() {
  ...
  try {
    ...
    const response = await fetch('https://api.brakebee.com/auth/refresh', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    document.cookie = `token=${data.token}; path=/; domain=.brakebee.com; secure; samesite=lax`;
    document.cookie = `refreshToken=${data.refreshToken}; path=/; domain=.brakebee.com; secure; samesite=lax`;
...
export function clearAuthTokens() {
  ...
  // Clear cookies
  document.cookie = 'token=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; domain=.brakebee.com';
  document.cookie = 'refreshToken=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; domain=.brakebee.com';
  document.cookie = 'csrf-token=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; domain=.brakebee.com';
  document.cookie = 'csrf-secret=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; domain=.brakebee.com';
...
export function getAuthToken() {
  ...
  if (localToken) {
    ...
    document.cookie = `token=${localToken}; path=/; domain=.brakebee.com; secure; samesite=lax`;
    return localToken;
  }

...
export async function refreshAuthToken() {
  ...
  try {
    ...
    document.cookie = `token=${data.token}; path=/; domain=${cookieConfig.domain}; ${cookieConfig.secure ? 'secure;' : ''} samesite=${cookieConfig.sameSite}`;
    document.cookie = `refreshToken=${data.refreshToken}; path=/; domain=${cookieConfig.domain}; ${cookieConfig.secure ? 'secure;' : ''} samesite=${cookieConfig.sameSite}`;
...
export async function startImpersonation(targetUserId, reason = '') {
  try {
    ...
    // Small delay to ensure token is saved before any redirects

...
export async function refreshAuthToken() {
  ...
  try {
    ...
    // Handle race condition: another tab already refreshed
    if (data.tokenAlreadyRefreshed) {
      // Return current token from storage (other tab updated it)
      return getAuthToken();
    }
...
export async function startImpersonation(targetUserId, reason = '') {
  try {
    ...
    localStorage.removeItem('refreshToken');
    document.cookie = `refreshToken=; path=/; domain=${cookieConfig.domain}; ${cookieConfig.secure ? 'secure; ' : ''}SameSite=${cookieConfig.sameSite}; max-age=0`;

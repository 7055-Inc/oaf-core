...
/**
 * Get all submissions (admin) with search, sort, pagination
 */
export async function getAllSubmissions(filters = {}) {
  ...
  if (filters.media_used !== undefined && filters.media_used !== '') params.append('media_used', filters.media_used);
...
/**
 * Toggle media_used flag (admin)
 */
export async function toggleMediaUsed(id, used) {
  const response = await authenticatedApiRequest(
    getApiUrl(`${MARKETING_BASE}/admin/submissions/${id}/media-used`),
    {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ media_used: used })
    }
  );

  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new Error(error.error || 'Failed to update media used flag');
  }
...
/**
 * Get download URL for original file (admin)
 */
export function getSubmissionDownloadUrl(id) {
  return getApiUrl(`${MARKETING_BASE}/admin/submissions/${id}/download`);
}

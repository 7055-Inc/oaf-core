...
const addImageOverlays = () => {
  const processImages = () => {
    ...
    images.forEach(img => {
      ...
      // Skip images in dashboard/admin areas - don't add overlay wrapper there
      const inDashboard = img.closest('.dashboard') !== null || 
                          img.closest('[class*="Dashboard"]') !== null ||
                          img.closest('[class*="slide-in"]') !== null ||
                          img.closest('[class*="widget"]') !== null ||
                          img.closest('table') !== null;
      if (inDashboard) {
        // Just add basic protection without wrapper
      ...
      // Ensure parent has position relative for absolute overlay to work
      const parentStyle = window.getComputedStyle(parent);
      if (parentStyle.position === 'static') {
        parent.style.position = 'relative';
      }
      
      // Create overlay directly (don't wrap the image - that breaks layout)
      ...
      // Add overlay as sibling after the image
      if (img.nextSibling) {

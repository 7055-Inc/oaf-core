...
export default function ProfileDisplay({ 
  ...
  // Fetch artist's products if they're a vendor (only if not provided via SSR)
  useEffect(() => {
    // Skip if we have SSR products or no user profile
    if (initialProducts.length > 0 || !userProfile?.id) {
    ...
    fetchProducts();
  }, [userProfile?.id, initialProducts.length]);

  // Fetch artist's event applications (v2)
  useEffect(() => {
    if (!userProfile?.id || userProfile?.user_type !== 'artist') return;

    const fetchArtistEvents = async () => {
      try {
        const data = await fetchArtistEventApplications(userProfile.id);
        setArtistEvents(Array.isArray(data) ? data : []);
      } catch (error) {

...
import styles from '../../../components/ProductReviews.module.css'; // Reuse product review styles
  ...
  const renderStars = (avg, count, onClick = null) => {
    ...
    for (let i = 1; i <= 5; i++) {
      stars.push(
        <span
          key={i}
          className={`${styles.star} ${i <= ratingVal ? styles.filled : ''} ${onClick ? styles.clickable : ''}`}

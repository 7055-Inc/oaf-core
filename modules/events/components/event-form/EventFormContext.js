/**
 * EventFormContext - v2 (canonical event create/edit form)
 *
 * React context for the accordion EventForm used at /dashboard/events/new and
 * /dashboard/events/[id]/edit. Uses v2 Events API (lib/events: fetchEventTypes,
 * fetchEvent, createEvent, updateEvent, application-fields, available-addons,
 * upload). Legacy my-events event-form and AddNew slide-in removed; create/edit
 * only via /dashboard/events/new and /dashboard/events/[id]/edit.
 */
...
export function EventFormProvider({ children, userData, eventId = null, initialData = null, claimedEventId = null }) {
  ...
  const saveDraft = useCallback(async () => {
    ...
    try {
      ...
      if (savedEventId) {
        ...
        await updateEvent(savedEventId, payload);
        return savedEventId;
      } else {
  ...
  // Save application fields - using v2 API
  const saveApplicationFields = useCallback(async (eventId) => {
    ...
    try {
      ...
      for (const field of applicationFields) {
        await addApplicationField(eventId, {
  ...
  const uploadImages = useCallback(async (files) => {
    setSaving(true);
    try {
      const urls = await uploadEventImages(files, savedEventId || 'new');
      setFormData(prev => ({
        ...prev,
  ...
  const addApplicationFieldLocal = useCallback((field) => {
  ...
  const updateApplicationFieldLocal = useCallback((fieldId, fieldName, value) => {
  ...
  const value = {
    ...
    addApplicationField: addApplicationFieldLocal,
    removeApplicationField,
    updateApplicationField: updateApplicationFieldLocal,

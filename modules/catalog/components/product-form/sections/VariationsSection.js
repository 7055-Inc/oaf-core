...
import { VariationBulkEditor } from '../..';
...
export default function VariationsSection() {
  ...
  const loadVariationTypes = async () => {
    try {
      ...
      setVariationTypes(Array.isArray(types) ? types : []);
    } catch (err) {
      // Silent fail
    }
  };
  ...
  const loadVariationValues = async (typeId) => {
    try {
      return await fetchVariationValues(typeId, savedProductId || undefined);
    } catch (err) {
      return [];
    }
  };
  ...
  const handleAddValue = async (typeId, valueName) => {
    ...
    try {
      const newValue = await createVariationValue(typeId, valueName, savedProductId);
      setSelectedTypes(prev => prev.map(type =>
        type.id === typeId ? { ...type, values: [...(type.values || []), newValue] } : type
      ));
    } catch (err) {
      // Silent fail
    }
  ...
  const handleCreateDraftChildren = async () => {
    ...
    try {
      ...
      for (let i = 0; i < selectedVariations.length; i++) {
        ...
        const response = await authenticatedApiRequest(getApiUrl('products'), {
          method: 'POST',
        ...
        for (const combo of combination) {
          if (combo.typeId && combo.valueId) {
            await createProductVariationRecord(createdProduct.id, combo.typeId, combo.valueId);
          }
        }
  ...
  const handleBulkEditorSave = async (finalizedVariations) => {
    ...
    try {
      ...
      for (const variation of finalizedVariations) {
        ...
        const response = await authenticatedApiRequest(getApiUrl(`products/${variation.id}`), {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },

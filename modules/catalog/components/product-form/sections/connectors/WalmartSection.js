...
import { fetchWalmartCategories, fetchWalmartProduct, saveWalmartProduct } from '../../../../../../lib/catalog';
...
export default function WalmartSection() {
  ...
  const loadWalmartData = async () => {
    ...
    setLoading(true);
    try {
      const data = await fetchWalmartProduct(savedProductId);
  ...
  const saveWalmartData = useCallback(async () => {
    ...
    try {
      ...
      const data = await saveWalmartProduct(savedProductId, payload);

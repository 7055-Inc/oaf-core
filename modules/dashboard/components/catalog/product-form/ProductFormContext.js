...
import { createProduct, updateProduct, updateProductStatus, fetchCategories, fetchCollections, createCollection } from '../../../../../lib/catalog';
...
export function ProductFormProvider({ children, userData, productId = null, initialData = null }) {
  ...
  const publishProduct = useCallback(async () => {
    ...
    try {
      ...
      // Use v2 API to update status
      await updateProductStatus(savedProductId, 'active');
  ...
  const loadCategories = useCallback(async () => {
    try {
      const categoryTree = await fetchCategories('tree');
      
      // Find "Shop" category and flatten its descendants for product assignment
      const shopCategory = categoryTree?.find(c => c.name === 'Shop');
      ...
      if (shopCategory) {
        ...
        // Fallback: use flat format
        const flatCategories = await fetchCategories('flat');
  ...
  // Load user/vendor collections
  const loadUserCategories = useCallback(async () => {
    try {
      const data = await fetchCollections('list');
  ...
  // Add new user/vendor collection
  const addUserCategory = useCallback(async (categoryData) => {
    try {
      const newCollection = await createCollection(categoryData);
      setUserCategories(prev => [...prev, newCollection]);
  ...
  // Load user addons from userData (already fetched via v2 API)
  const loadUserAddons = useCallback(() => {
    // Addons are included in userData from the v2 users API
    if (userData?.addons) {
      setUserAddons(userData.addons);
    }
  }, [userData]);

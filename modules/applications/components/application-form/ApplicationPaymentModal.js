/**
 * Application payment modal – Stripe payment for application/jury fees.
 * Uses lib/applications/api.js (createApplicationPaymentIntent, confirmApplicationPayment).
 */

import { useState, useEffect } from 'react';
import { createApplicationPaymentIntent, confirmApplicationPayment } from '../../../../lib/applications/api';
...
export default function ApplicationPaymentModal({ application, event, onSuccess, onCancel }) {
  ...
  useEffect(() => {
    ...
    const createIntent = async () => {
      try {
        const { client_secret } = await createApplicationPaymentIntent(application.id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (paymentIntent?.status === 'succeeded') {
        await confirmApplicationPayment(application.id, paymentIntent.id);
        onSuccess();
      }
    } catch (err) {
      setError(err.message);

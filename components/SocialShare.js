...
export default function SocialShare({ 
  ...
  const platforms = [
    ...
    { key: 'email', label: 'Email', color: '#6c757d' },
    { key: 'sms', label: 'SMS', color: '#28a745' },
    { key: 'copy', label: copied ? 'Copied!' : 'Copy Link', color: '#6c757d' }
  ];
      ...
      <div className={styles.shareButtons}>
        {platforms.map(({ key, label, color }) => (
          <button
            ...
            className={`${styles.shareButton} ${copied && key === 'copy' ? styles.copied : ''}`}
            title={label}
            style={{ '--button-color': color }}
          >
            {icons[key]}
            {!compact && <span className={styles.shareLabel}>{label}</span>}
          </button>
        ))}
      </div>
    </div>

/**
 * SOP Documents Service
 * Core CRUD operations for SOP documents
 */
...
async function getBreadcrumb(sop) {
  ...
  while (folderId) {
    const [rows] = await pool.execute(
      'SELECT id, title, parent_id FROM sop_folders WHERE id = ?',
      [folderId]
    );
    const folder = rows[0];
    if (!folder) break;
...
async function update(id, data, updated_by = null) {
  ...
  const jsonFields = [
    'standard_workflow', 'exit_points', 'escalation', 
    'transfer', 'additional_information', 'related_sop_ids'
  ];

/**
 * SOP Documents Routes
 * CRUD operations for SOP documents
 */

const express = require('express');
const { sopsService } = require('../services');
 ...
 * Query params: folder_id, status, search, limit, offset
 */
router.get('/', async (req, res) => {
  try {
    ...
    if (folder_id !== undefined) {
      filters.folder_id = folder_id === '' || folder_id === 'null' ? null : folder_id;
    }
...
/**
 * GET /api/v2/sop/sops/:id
 * Get a specific SOP with breadcrumb
 */
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const changeSummary = prev 
      ? `Changed from: ${prev.status}, to: ${sop.status} by ${updated_by}` 
      : null;
...
/**
 * GET /api/v2/sop/sops/:id/versions
 * Get version history for an SOP
 */
router.get('/:id/versions', async (req, res) => {
  try {
    const limit = req.query.limit ? parseInt(req.query.limit, 10) : 50;
    const list = await sopsService.getVersions(req.params.id, limit);
    res.json({ success: true, data: list });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }

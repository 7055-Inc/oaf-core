/**
 * Amazon Connector Routes (v2)
 * Mounted at /api/v2/catalog/amazon
 *
...
router.get('/oauth/callback', async (req, res) => {
  try {
    ...
    if (!spapi_oauth_code || !state) {
      return res.redirect(`${process.env.FRONTEND_URL}/dashboard/catalog/addons/amazon?amazon_error=missing_params`);
    }
    const result = await amazonService.handleOAuthCallback(spapi_oauth_code, state, selling_partner_id);
    if (result.success) {
      return res.redirect(`${process.env.FRONTEND_URL}/dashboard/catalog/addons/amazon?amazon_status=connected&shop_id=${encodeURIComponent(result.shop_id)}`);
    }
    return res.redirect(`${process.env.FRONTEND_URL}/dashboard/catalog/addons/amazon?amazon_error=connection_failed`);
  } catch (err) {
    console.error('Amazon OAuth callback error:', err);
    res.redirect(`${process.env.FRONTEND_URL}/dashboard/catalog/addons/amazon?amazon_error=server_error`);
  }
});

router.get('/oauth/authorize', requireAuth, async (req, res) => {
  try {
    const result = amazonService.oauthAuthorize(req.userId);
...
router.get('/shops', requireAuth, async (req, res) => {
  try {
    const shops = await amazonService.getShops(req.userId);
...
router.post('/shops/:shopId/disconnect', requireAuth, async (req, res) => {
  try {
    await amazonService.disconnectShop(req.params.shopId, req.userId);
...
router.get('/products', requireAuth, async (req, res) => {
  try {
    const products = await amazonService.listProducts(req.userId);
...
router.post('/products/:productId', requireAuth, async (req, res) => {
  try {
    const result = await amazonService.saveProduct(req.params.productId, req.userId, req.body);
    if (!result.found) return res.status(404).json({ success: false, error: { message: 'Product not found' } });
    return res.json({ success: true, data: { message: 'Amazon product data saved' } });
  } catch (error) {
...
router.get('/inventory', requireAuth, async (req, res) => {
  try {
    const allocations = await amazonService.getInventory(req.userId);
...
router.post('/inventory/:productId', requireAuth, async (req, res) => {
  try {
    const result = await amazonService.updateInventoryAllocation(req.params.productId, req.userId, req.body.allocated_quantity);
    if (!result.found) return res.status(404).json({ success: false, error: { message: 'Product not found' } });
...
router.post('/allocations/bulk', requireAuth, async (req, res) => {
  try {
    const result = await amazonService.bulkAllocations(req.userId, req.body.allocations);
...
router.get('/logs', requireAuth, async (req, res) => {
  try {
    const logs = await amazonService.getLogs(req.userId, req.query);
...
router.post('/sync/product/:productId', requireAuth, async (req, res) => {
  try {
    ...
    const result = await amazonService.syncProductToAmazon(req.params.productId, req.userId, shop_id);
    return res.json({ success: true, data: { message: 'Product synced to Amazon', result } });
  } catch (error) {
...
router.post('/sync/orders', requireAuth, async (req, res) => {
  try {
    const { shop_id } = req.body;
    if (!shop_id) return res.status(400).json({ success: false, error: { message: 'shop_id required' } });
    const result = await amazonService.syncOrdersFromAmazon(req.userId, shop_id);
...
router.get('/corporate/products', requireAuth, async (req, res) => {
  try {
    const products = await amazonService.listCorporateProducts(req.userId);
...
router.get('/corporate/products/:productId', requireAuth, async (req, res) => {
  try {
    const product = await amazonService.getCorporateProduct(req.params.productId, req.userId);
...
router.post('/corporate/products/:productId', requireAuth, async (req, res) => {
  try {
    const result = await amazonService.saveCorporateProduct(req.params.productId, req.userId, req.body);
...
router.delete('/corporate/products/:productId', requireAuth, async (req, res) => {
  try {
    const result = await amazonService.removeCorporateProduct(req.params.productId, req.userId);
    if (!result.found) return res.status(404).json({ success: false, error: 'Product not found' });
...
router.get('/admin/products', requireAuth, requirePermission('manage_system'), async (req, res) => {
  try {
    ...
    const result = await amazonService.adminListProducts({ status, page: parseInt(page), limit: parseInt(limit), search });
...
router.post('/admin/products/:productId/activate', requireAuth, requirePermission('manage_system'), async (req, res) => {
  try { await amazonService.adminActivate(req.params.productId); return res.json({ success: true, data: { message: 'Activated' } }); }
...
router.post('/admin/products/:productId/pause', requireAuth, requirePermission('manage_system'), async (req, res) => {
  try { await amazonService.adminPause(req.params.productId); return res.json({ success: true, data: { message: 'Paused' } }); }
  catch (error) { return res.status(500).json({ success: false, error: { message: 'Failed' } }); }
});

router.put('/admin/products/:productId', requireAuth, requirePermission('manage_system'), async (req, res) => {
  try { await amazonService.adminUpdateProduct(req.params.productId, req.body); return res.json({ success: true, data: { message: 'Updated' } }); }
  catch (error) { return res.status(500).json({ success: false, error: { message: 'Failed' } }); }
});
...
router.get('/admin/corporate/products', requireAuth, requirePermission('manage_system'), async (req, res) => {
  try {
    const { status = 'all', page = 1, limit = 25, search = '' } = req.query;
    const result = await amazonService.adminListCorporateProducts({ status, page: parseInt(page), limit: parseInt(limit), search });
...
router.post('/admin/corporate/products/:productId/activate', requireAuth, requirePermission('manage_system'), async (req, res) => {
  try { await amazonService.adminActivateCorporate(req.params.productId, req.userId); return res.json({ success: true, message: 'Corporate product activated' }); }
...
router.post('/admin/corporate/products/:productId/pause', requireAuth, requirePermission('manage_system'), async (req, res) => {
  try { await amazonService.adminPauseCorporate(req.params.productId, req.userId); return res.json({ success: true, message: 'Corporate product paused' }); }
...
router.post('/admin/corporate/products/:productId/reject', requireAuth, requirePermission('manage_system'), async (req, res) => {
  try { const { reason } = req.body; await amazonService.adminRejectCorporate(req.params.productId, req.userId, reason); return res.json({ success: true, message: 'Corporate product rejected' }); }
...
router.put('/admin/corporate/products/:productId', requireAuth, requirePermission('manage_system'), async (req, res) => {
  try { await amazonService.adminUpdateCorporateProduct(req.params.productId, req.body); return res.json({ success: true, message: 'Corporate product updated' }); }

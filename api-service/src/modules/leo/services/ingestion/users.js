/**
 * Leo AI - User Ingestion Service
 * 
 * Ingests users with all profile data into ChromaDB:
 ...
 * 
 * Uses shared database pool from api-service (no separate credentials needed)
 */
...
class UserIngestion {
   ...
   * @param {string} lastRun - ISO timestamp, only get users updated after this
   */
  ...
  parseJSON(jsonString) {
    ...
    try {
      ...
      logger.warn('Failed to parse JSON:', { value: String(jsonString).substring(0, 50) });
      return null;
    }
  }

  /**
   * Build user content for embedding (searchable text)
   */
  ...
  async ingestUser(user) {
    try {
      ...
      // Update stats
      this.stats.total++;
      if (this.stats[user.user_type] !== undefined) {
   ...
   * @param {string} lastRun - ISO timestamp, only ingest users updated after this
   */
...
function getUserIngestion() {
  if (!instance) {
    instance = new UserIngestion();
  }
  return instance;
}
